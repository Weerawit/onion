...
public class BaseFormController implements ServletContextAware {
	...
	protected int getPageSize(HttpServletRequest request) {
		...
		builder.append(" >30</option>");
		builder.append("<option value='50'");
		if (ps == 50) {
			builder.append(" selected");
		}
		builder.append(" >50</option>");
		builder.append("<option value='100'");
		if (ps == 100) {
			builder.append(" selected");
		}
		builder.append(" >100</option>");
		builder.append("</select>");

		request.setAttribute("psSelect", builder.toString());
		return ps.intValue();
	}
	...
	protected String getSortColumn(HttpServletRequest request) {
		String column = "";
		Enumeration<String> paramNames = request.getParameterNames();
		while (paramNames.hasMoreElements()) {
			String name = (String) paramNames.nextElement();
			if (name != null && name.startsWith("d-") && name.endsWith("-s")) {
	...
	protected String getSortOrder(HttpServletRequest request) {
		...
		while (paramNames.hasMoreElements()) {
			String name = (String) paramNames.nextElement();
			if (name != null && name.startsWith("d-") && name.endsWith("-o")) {
